...
# -*- coding: utf-8 -*-
"""
This is a code to compute the optic flow for Figure 5C for the vertical rotation. It also computes the optic flow for different stretches and 
...
for ss in range(len(Stretches)):
   ...
   #%%
    ...
    Video,ex_idx=make_rotating_stim(pos1,pos2,vec1,-vec2,name=f'Vertical_rt_im_stretch_{stretch_factor_X}',sz_x=sz_x,sz_y=sz_y,scale=.01)
    ...
    curlw1,curlw2=rotate(curlw1,curlw2)
    divw1,divw2=rotate(divw1,divw2)
    ...
    pyr_flow=Heeger_pyr_flow(UorA='U_pyr',
                    sname=current_folder+f'/Images/Steer_pyr_opticflow_V_ortho_im_stretch_{stretch_factor_X}',
                    vfile=current_folder+'/Videos/',
                    name=f'Optic_flow__V_ortho_im_stretch_{stretch_factor_X}',scale=.5,fps=10,smoothness=False,alpha=100,ex_idx=ex_idx,num_scales=2)
    ...
    pyr_flow=Heeger_pyr_flow(UorA='A_pyr',
                    sname=current_folder+f'/Images/Steer_pyr_opticflow_V_aniso_ortho_im_stretch_{stretch_factor_X}',
                    vfile=current_folder+'/Videos/',
                    name=f'Optic_flow_aniso_V_ortho_im_stretch_{stretch_factor_X}',scale=.5,fps=10,smoothness=False,alpha=100,ex_idx=ex_idx,num_scales=2,num_cells=num_cells)
